// src/app/(cms)/admin/tours/edit/[id]/page.tsx
'use client';
...
import { getTourPageTourById, updateTourPageTour } from '@/lib/firebase/firestore';
import type { Tour, GalleryImage, AdditionalSection } from '@/lib/types';
...
const tourFormSchema = z.object({
  name: z.string().min(3, 'Tour name is required.'),
...
export default function EditTourPage() {
  ...
  useEffect(() => {
    ...
    const fetchTour = async () => {
      try {
        const tourData = await getTourPageTourById(id);
        if (tourData) {
          form.reset({
            ...tourData,
            gallery: tourData.gallery || [],
            additionalSections: tourData.additionalSections || [],
          });
        } else {
           toast({ title: 'Error', description: 'Tour not found.', variant: 'destructive' });
           router.push('/admin/tours/tours-list');
        }
      } catch (error) {
        console.error('Failed to fetch tour', error);
        toast({ title: 'Error', description: 'Failed to load tour data.', variant: 'destructive' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (data: TourFormValues) => {
    ...
    try {
      await updateTourPageTour(id, data);
      toast({
        title: 'Tour Updated',
        description: 'The tour details have been successfully saved.',
      });
    } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex justify-between items-center">
             <Button variant="outline" asChild>
                <Link href="/admin/tours/tours-list">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-headline font-light">Main Tour Details</CardTitle>
            <CardDescription>This is the primary information for the tour listing.</CardDescription>
          </CardHeader>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>Add images for the tour detail page slider.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
