// src/app/(cms)/admin/services/services-list/page.tsx
'use client';
...
import { getServicePageServices, addServicePageService, deleteServicePageService, updateServicePageService } from '@/lib/firebase/firestore';
...
export default function ServicesListPage() {
  ...
  const onSubmit = async (data: ServiceFormValues) => {
    ...
    try {
      if (editingService && editingService.id) {
        await updateServicePageService(editingService.id, data);
      } else {
        await addServicePageService(data);
      }
      form.reset();
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteServicePageService(id);
    } catch (error) {
  ...
  return (
    <div className="grid gap-6">
       <Card>
        <CardHeader>
            <CardTitle className="font-headline font-light">Services Page List</CardTitle>
            <CardDescription>
                Manage the services displayed on the "Our Services" page.
            </CardDescription>
        </CardHeader>
      </Card>
      <Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline font-light">Current Services Page Services</CardTitle>
          <CardDescription>Manage the existing services.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
